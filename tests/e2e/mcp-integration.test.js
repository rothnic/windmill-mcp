...
describe.skipIf(!isE2EEnabled)("MCP Server E2E Tests", () => {
  ...
  beforeAll(async () => {
    try {
      ...
      // Verify server is responsive after connection
      const pingResponse = await mcpClient.request(
        {
          method: "tools/list",
          params: {},
        },
      ...
      throw new Error(
        `MCP Server initialization failed: ${error.message}. ` +
          `Check that the server builds correctly and starts without errors.`,
      );
    }
  ...
  describe("Script Tools", () => {
    it("should have script-related tools available", async () => {
      ...
      const hasScriptTools = toolNames.some((name) =>
  ...
  describe("Workflow Tools", () => {
    it("should have workflow-related tools available", async () => {
      ...
      const hasWorkflowTools = toolNames.some(
        (name) =>
...
describe("MCP E2E Test Configuration", () => {
  ...
  it("should skip MCP E2E tests when not configured", () => {
    if (!isE2EEnabled) {
      console.log(
        "MCP E2E tests skipped - set E2E_WINDMILL_URL and E2E_WINDMILL_TOKEN to enable",
      );
    }
...
// Smoke tests - verify server starts and responds
describe.skipIf(!isE2EEnabled)("MCP Server Initialization Smoke Tests", () => {
  let mcpClient;
  let transport;
  ...
  beforeAll(async () => {
    try {
      ...
      throw new Error(
        `Failed to create MCP client for smoke test: ${error.message}`,
      );
    }
  }, 15000);
  ...
  it("should start server without fatal errors", async () => {
    // If we got here, the server started successfully in beforeAll
    expect(mcpClient).toBeDefined();
    expect(transport).toBeDefined();
  });

  it("should respond to tools/list within timeout", async () => {
    ...
    // Server should respond quickly (< 5 seconds)
    expect(duration).toBeLessThan(5000);
  });

  it("should have valid tool schemas after initialization", async () => {
    const response = await mcpClient.request({
      method: "tools/list",
      params: {},
    });

    // Verify at least one tool has proper schema structure
    const toolWithSchema = response.tools.find(
  ...
  it("should establish stable connection for multiple requests", async () => {
    // Make 3 rapid requests to verify connection stability
    const requests = [
      mcpClient.request({ method: "tools/list", params: {} }),
      mcpClient.request({ method: "tools/list", params: {} }),
      mcpClient.request({ method: "tools/list", params: {} }),
    ];
